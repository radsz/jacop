 ...
 * @version 4.10
 */
public class PplusQeqR extends PrimitiveConstraint implements FloatDerivableConstraint {
  ...
  public FloatVar derivative(Store store, FloatVar f, Set<FloatVar> vars, FloatVar x) {
    if (f.equals(r)) {
      ...
      Derivative.poseDerivativeConstraint(
          new PplusQeqR(
              Derivative.getDerivative(store, p, vars, x),
              Derivative.getDerivative(store, q, vars, x),
              v));
      return v;
      ...
      Derivative.poseDerivativeConstraint(
          new PminusQeqR(
              Derivative.getDerivative(store, r, vars, x),
              Derivative.getDerivative(store, q, vars, x),
              v));
      return v;
