...
public interface ClauseDatabaseInterface {
  ...
  /**
   * It adds a clause to the database. This can change the state of the solver, for example if the
   * clause is unsatisfiable in the current trail affectation, the solver will get in the conflict
   * state.
   *
  ...
  /**
   * It informs the ClausesDatabase that this literal is set, so that it can perform unit
   * propagation.
   *
  ...
  /**
   * It returns the clause obtained by resolution between clauseIndex and clause. It will also
   * modify in place the given SetClause (avoid allocating).
   *
