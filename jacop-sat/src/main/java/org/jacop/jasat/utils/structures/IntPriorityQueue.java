...
public final class IntPriorityQueue {
  ...
  private Node findLastNode() {
    ...
    while (depth > 0) {
      ...
      if (current.leftSon == null) {
        ...
        current =
            current.leftSon.depth > current.rightSon.depth ? current.leftSon : current.rightSon;
      }
      depth--;
    }
  ...
  /**
   * balances the tree again so that the given node moves up (to be called when the current node has
   * highest priority than its parent)
   *
   * @param n the node
   */
