...
public class SetDomainValueEnumeration extends ValueEnumeration {
  ...
  /**
   * Constructs an enumeration for the specified SetDomain.
   *
  ...
  /**
   * The number of elements at each level is described by Pascal's-triangle. Example: domain =
   * {{}..{1..3} glb.size = 0 lub.size = 3 the level(pascalLevel) in Pascal's-triangle: level = 3-0
   * (lub.size-glb.size) This level(pascalLevel) in Pascal's-triangle is 1 3 3 1 And we get the
   * elements: level 0 : {} level 1 : {1}, {2}, {3} level 2 : {1,2}, {1,3}, {2,3} level 3 : {1,2,3}
   * So getPascal(2,2) returns the Set {1,3}
   *
   * <p>occLevel = The level in Pascal's triangle that describes how many times the first value
   * occurs.
   *
   * <p>occPos = the position in Pascal's triangle on row occLevel that describes how many times the
   * first value occurs
   *
   * <p>occ = the number of times the first value occurs.
   *
