...
public class Lex extends Constraint implements Stateful {
  ...
  public void consistency(Store store) {
    ...
    if (a.domain.glb().isEmpty()) {
            ...
            if (!enumerGLBofB.hasMoreElements())
              if (enumerLUBofA.hasMoreElements()) {
                ...
                if ((strict && nextElinLUBofA < b.domain.lub().max())
                  ...
                  // lex can be achieved either by making it empty or not adding any elements above
                  // lastElinLUBofA.
                  a.domain.inLUB(
                      store.level, a, new IntervalDomain(Integer.MIN_VALUE, lastElinLUBofA));
                  return;
                }
              }
      ...
      if (b.domain.glb().isEmpty()) {
        ...
        if (strict
            && a.domain.glb().max() == a.domain.lub().max()
            && a.domain.card().min() == 1
            && a.domain.card().max() == 2
            && b.domain.lub().max() == a.domain.glb().max()
            && b.domain.card().max() == 2) {
        ...
        // count how many smaller elements (noSmaller) given current value can be added from a.lub
        // to a.glb.
        int noSmaller = 0;
          ...
          if (nextElinGLBofA == nextElinGLBofB) {
            ...
            if (!enumerGLBofB.hasMoreElements()) {
              ...
              if (noSmaller == 0) {
                ...
                if (strict) {
                  // there is at least one element in bLUB which can be added to bGLB to satisfy a
                  // <lex b.
            ...
            if (noSmaller == 1) {
              ...
              if (nextElinLUBofA == nextElinGLBofB) {
                ...
                if (!enumerGLBofB.hasMoreElements()) {
                  ...
                  // TODO
                  // if there is no element which can be added to B to find 2nd way of making a <lex
                  // b
                  // then enforce the first way.
                  // 2nd way == true, if and only if next(enumerLUBofA) < b.domain.lub.max() (for
                  // strict case, nonstrict uses <= ).
                  return;
                }
                ...
                while (true) {
                  ...
                  if (nextElinLUBofA < nextElinGLBofB) {
                    ...
                    // nextElinLUBofA == nextElinGLBofB, still not decided if there is a second way
                    // to fix it.

                    if (!enumerGLBofB.hasMoreElements()) {

                      // TODO
                      // if there is no element which can be added to B to find 2nd way of making a
                      // <lex b
                      // then enforce the first way.
                      // 2nd way == true, if and only if next(enumerLUBofA) < b.domain.lub.max()
                      // (for strict case, nonstrict uses <= ).
                      return;
                    }
  ...
  public void impose(Store store) {
    ...
    assert (!b.domain.lub().contains(Integer.MIN_VALUE))
        : "Lex constraint does not allow Integer.MIN_VALUE in the domain";
