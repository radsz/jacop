...
/**
 * Registry for managing cost variable and domain operation handlers.
 * This registry allows modules (jacop-floats, jacop-sets) to register their
 * type-specific handlers, enabling search algorithms to work with different
 * variable types without direct dependencies.
 * <p>
 * Handlers are registered in order of priority. When looking up a handler,
 * the first applicable handler is returned.
 *
...
public class SearchHandlerRegistry {
    ...
    /**
     * Registers a cost variable handler.
     * Handlers are checked in registration order, so more specific handlers
     * should be registered after general ones.
     *
    ...
    /**
     * Registers a domain operation handler.
     * Handlers are checked in registration order, so more specific handlers
     * should be registered after general ones.
     *
     * @param handler the handler to register
    ...
    /**
     * Resets the registry to its default state (only IntVar handlers).
     * Useful for testing.
     */
