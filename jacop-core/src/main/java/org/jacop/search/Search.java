...
/**
 * All searches needs to implement this interface in order to be manipulable by a large variety of
 * search listeners. Of course, the search which implements this interface will need to call
 * appropriate functions of attached listeners in the right place and act accordingly to the output
 * of listeners.
 *
...
public interface Search<T extends Var> {
  ...
  /**
   * It specifies the sub-searches for the current search. In order for the current search to
   * succeed at least one of those must succeed. If there are no sub-searches then the current
   * search succeeds if all variables within select choice point object are assigned and all
   * constraints attached to those variables are satisfied.
   *
  ...
  /**
   * It performs search, first by setting up the internal items/attributes of search, followed later
   * by a call to labeling function with argument specifying the index of the first not grounded
   * variable.
   *
   * @return true if the solution was found.
   */
   ...
   * aim at finding an optimal solution.
   *
   ...
   * about the master search. For example, the textual description of the solution.
   *
