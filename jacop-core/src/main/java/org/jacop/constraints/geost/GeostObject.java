/**
 * GeostObject.java This file is part of JaCoP.
 *
...
public class GeostObject {
  ...
  /**
   * It is executed as soon as backtracking has occurred making previously grounded variable
   * ungrounded again.
   *
    ...
    public boolean consistencyStartPlusDurationEqEnd(Store store) {
      ...
      do {
        ...
        if (start.singleton()) {
          ...
          start.domain.in(
              store.level, start, end.min() - duration.max(), end.max() - duration.min());

          duration.domain.in(
              store.level, duration, end.min() - start.max(), end.max() - start.min());
