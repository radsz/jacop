...
public abstract class InternalConstraint {
   ...
   * constraint. This point must be larger or equal (resp. smaller or equal) to the
   ...
   * into account)
   *
   * <p>The dimension of the point returned is k+1, where k is the object dimension. The last
   * dimension is time.
   *
   * @param minlex defines whether the maximal or minimal point should be returned
   * @return the infeasible point's coordinates. If constraint cannot generate outbox then it
   *     returns null.
   */
  public abstract int[] absInfeasible(Geost.SweepDirection minlex);

  /**
   * It provides information about the constraint future. If a constraint will always generate the
  ...
  /**
   * It provides an approximation of the number of infeasible points enforced by this constraint
   * only. The information provided by this function cannot be accurate, since no object is passed
   * as an argument, but some consistent approximation should exist. For instance, in the case of a
   * forbidden area, the returned value can be the number of points included in the area.
   *
   * <p>This information is used as a heuristic in the sweeping algorithm to decide which constraint
   * to use, so that the constraints that cover the largest space are used first.
   *
  ...
  /**
   * It provides a collection, possibly empty, of variables which define this constraint. This
   * information is used to build a reverse index that allows to update the absolute infeasible
   * points of a constraint when a variable changes.
   *
