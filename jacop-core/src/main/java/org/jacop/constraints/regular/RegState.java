...
public abstract class RegState {
  ...
  /**
   * It specifies that for a given value an automata will move from the current state to the
   * successor state.
   *
   * @param suc successor state
  ...
  /**
   * The function return if the state is still active. It depends on how many active levels remains
   * for state level and the position of the state.
   *
   * @param activeLevels -
