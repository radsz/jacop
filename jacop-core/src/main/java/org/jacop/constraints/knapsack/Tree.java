...
public class Tree {
  ...
  /**
   * It specifies the leaf containing the critical item. The critical item is the one used partially
   * by Linear Programming approach.
   */
  public TreeLeaf criticalLeaf;

  /**
   * It specifies the leaf containing the last right item which is being used during
   * computeMandatory(). if max() of the right item after criticalRight is changed then it can be
   * safely ignored.
   */
  public int criticalRightLeaf;

  /**
   * It specifies the leaf containing the left most item which is being used during
   * computeForbidden(). if min() of the left item before criticalLeft is changed then it can be
   * safely ignored.
   */
  ...
  /**
   * It specifies the fraction of the critical item which has been not included in the optimal
  ...
  /**
   * It specifies the already obtained profit due to the items which are already included in the
   * solution.
   */
  public int alreadyObtainedProfit;

  /**
   * It specifies the already used capacity due to the items which are already included in the
   * solution.
   */
  ...
  /**
   * It constructs a tree out of the list of items and creates proper supporting structures. It
   * assumes the list of items is greater than 1.
   *
  ...
  /**
   * It updates information about the critical item, as well as information about fraction of
   * critical item which is not taken.
   *
  ...
  /**
   * It finds a leaf which reaches the limit of the given capacity. Items weight is added from the
  ...
  /**
   * It returns the last (the least efficient) item in the tree. It is a starting leaf for
   * computeForbidden() part.
   *
   ...
   * @param startingPosition it specifies the first leaf in the array which has not been updated
   *     before.
   */
   ...
   * items given the amount of profitSlack.
   *
   ...
   * @param profitSlack the amount of reserve profit which can be sacrificed before violating the
   *     constraint.
   * @return the amount of weight of a given item that can be replaced by Right items without
   *     violating the constraint.
   */
  public int computeReplacableWeight(
    ...
    // Not the whole slackProfit used and not the whole of item being checked is covered yet. Using
    // item on the right (if exists) to use remaining
    // profit.
    ...
    if (!exhaustedRightItems) {
      ...
      double efficiencyLoss =
          profitOfItemChecked / (double) weightOfItemChecked
              - currentNode.rightNeighbor.getPSum() / (double) currentNode.rightNeighbor.getWSum();
  ...
  /**
   * It finds next leaf of a maximum weight of at least weight, so it can have some parts of it
   * mandatory.
   *
  ...
  public void initializeComputeForbidden() {
    ...
    profitFromCriticalTaken =
        (criticalLeaf.getWSum() - availableWeightOfCriticalItem)
            * criticalLeaf.profitOfOne
            / (double) criticalLeaf.weightOfOne;
    currentNode = criticalLeaf;
   ...
   *     violating the constraint.
   */
  public int computeIntrusionWeight(
      int weightOfItemChecked,
      int maxNoOfItems,
      int profitOfItemChecked,
      double efficiencyOfItemChecked,
    ...
    // Not the whole slackProfit used and not the whole of item being checked is covered yet. Using
    // item on the left (if exists) to use remaining
    // profit.
  ...
  /**
   * It finds previous leaf of a maximum weight of at least weight, so it can have some parts of it
   * forbidden.
   *
  ...
  /**
   * It computes the minimum of capacity variable for knapsack constraint given the minimum
   * requirement for profit.
   *
   * @param minProfit minimum profit obtained by the knapsack.
   * @return it returns the minimum required weight to satisfy min profit requirements.
   */
  public int computeMinWeight(int minProfit) {
    ...
    int minWeightForCurrentTree =
        (int)
            Math.ceil(
                remainingProfitToFind * finalLeaf.weightOfOne / (double) finalLeaf.profitOfOne);
  ...
  public int computeMinProfit(int minWeight) {
    ...
    while (!current.isLeaf()) {
      ...
      if (current.left.getWSum() == 0) {
        ...
        current = current.left;
      } else if (usedCapacity + current.right.getWSum() <= minWeight) {
    ...
    int minProfitForCurrentTree =
        (int)
            Math.ceil(
                remainingWeightToFind * finalLeaf.profitOfOne / (double) finalLeaf.weightOfOne);
