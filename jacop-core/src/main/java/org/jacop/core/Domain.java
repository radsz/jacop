...
public abstract class Domain {
  ...
  /**
   * It specifies the constant responsible of conveying a message that no information is provided by
   * the domain when describing the change which has occurred within the domain.
   */
  public static final int NOINFO = Integer.MIN_VALUE;

  /**
   * It specifies the constant for NONE event, if event is NONE then the constraint is not attached
   * to a variable. Useful for constraints which are always satisfied or not satisfied after the
   * first consistency function execution.
   */
   ...
   * positions are not satisfied yet.
   */
  ...
  /**
   * It specifies if the vector of constraints were cloned (if it was not cloned then the same
   * vector is reused across domains with different stamps. Only reading actions are allowed on not
   * cloned vector of constraints.
   */
  ...
  /**
   * It removes the specified level. This function may re-instantiate the old copy of the domain
   * (previous value) or recover from changes done at stamp level to get the previous value at level
   ...
   * Variable object. putConstraint function from Variable must make a copy of a list of model
  ...
  /**
   * It adds a constraint to a domain, it should only be called by putConstraint function of
   * Variable object. putConstraint function from Variable must make a copy of a list of search
   * constraints if vector was not cloned.
   *
   ...
   * satisfied.
   *
   * @return the number of constraints attached to the original domain of the variable associated
   *     with this domain.
   */
  public abstract int sizeConstraintsOriginal();

  /**
   * It returns all the constraints attached currently to the domain. It should not be used
   * extensively.
   *
