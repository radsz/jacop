...
public class SmallDenseDomain extends IntDomain implements Cloneable {
  ...
  public SmallDenseDomain(int min, long bits) {
    ...
    if (bits != 0) {
      ...
      if (size == 1) this.singleton = true;
      else this.singleton = false;
  ...
  public boolean contains(IntDomain domain) {
    ...
    if (domain.domainID() == IntDomain.SmallDenseDomainID) {
      ...
      if (bitsResult != this.bits) return false;
      else return true;
    }
    ...
    */
    ...
    assert (result == this.toIntervalDomain().contains(domain))
        : "Improper implementation of function contains " + this + "d" + domain;
  ...
  public boolean eq(IntDomain domain) {
    ...
    if (domain.domainID() == IntDomain.SmallDenseDomainID) {
      ...
      if (input.min == this.min && input.bits == this.bits) return true;
      else return false;
    }
  ...
  public int intersectAdapt(int min, int max) {
    ...
    this.max = max;
    this.size = getSize(bits);
    if (this.size == 1) this.singleton = true;
    else this.singleton = false;
  ...
  public boolean contains(int min, int max) {
    ...
    if (max - min + 1 != this.getSize(result)) return false;
