...
public class SimpleBacktrackableManager implements BacktrackableManager {
  ...
  /**
   * It contains indexes of objects changed at active level. It may be empty if
   * trailContainsAllChanges is set to true.
   */
  ...
  /**
   * It specifies the cutoff value after which a trail is no longer stored and recovery of the old
  ...
  public void update(Backtrackable[] objects, int noOfObjects) {
    ...
    System.arraycopy(
        currentlyChanged.dense, 0, replacement.dense, 0, currentlyChanged.dense.length);
    System.arraycopy(
        currentlyChanged.sparse, 0, replacement.sparse, 0, currentlyChanged.sparse.length);
