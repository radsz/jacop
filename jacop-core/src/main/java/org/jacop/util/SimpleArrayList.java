...
/**
 * Different implementation of an ArrayList data structures. This version is tailored for JaCoP. Use
 * with care, check when it uses == instead of equals().
 *
...
public class SimpleArrayList<V> extends AbstractList<V> {
   ...
   * returned by the collection's iterator. The {@code ArrayList} instance has an initial capacity
  ...
  public SimpleArrayList(Collection<? extends V> c) {
    ...
    elementData = (V[]) new Object[(int) Math.min((size * 110L) / 100, Integer.MAX_VALUE)];
  ...
  /**
   * AbstractList defines hashCode so that it depends on the objects included. This makes it costly
   * (linear in the number of elements).
   *
   * <p>Taking the hash of elementData will make it faster TODO make sure this is what we want
   */
  @Override
  public int hashCode() {
  ...
  /**
   * AbstractList defines equals so that it depends on the objects included. This makes it costly
   * (linear in the number of elements).
   *
  ...
  /**
   * same as clear(), but references to objects are kept internally. This allows the operation to be
   * constant time, but implies that objects will not be garbage collected until their references
   * are overwritten to store other objects.
   */
  ...
  /**
   * Searches for the first occurrence of the given argument, testing for equality using the {@code
   * equals} method.
   *
   * @param elem an object.
   * @return the index of the first occurrence of the argument in this list; returns value -1 if the
   *     object is not found.
   */
   ...
   * operation). More formally, removes an element {@code e} such that {@code (o==null ? e==null :
   * o.equals(e))}, if the list contains one or more such elements. Returns {@code true} if the list
   * contained the specified element (or equivalently, if the list changed as a result of the call).
   *
   * <p>
   *
   ...
   * <i>only</i> if the caller knows that the list does not contain any {@code null} elements.
   *
