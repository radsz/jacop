...
public class Golomb extends ExampleFd {
  ...
  public void model() {
    ...
    for (int i = 1; i < numbers.length; i++) {
      ...
      for (int j = 0; j < i; j++) {
        ...
        IntVar sub =
            new IntVar(store, "c" + i + "_" + j, (i - j) * (i - j + 1) / 2, noMarks * noMarks);
  ...
  /**
   * It specifies specific search for the optimal solution search procedure, which printouts
   * intermediate search results and shows how the search is progressing.
   *
