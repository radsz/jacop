...
public class BreakingNews extends ExampleFD {
  ...
  public void model() {
    ...
    store.impose(
        new Or(
            new XneqY(story[i30pound], location[iNewHope]),
    ...
    store.impose(
        new Or(
            new And(
                new XeqY(story[iblimp], reporter[iLous]),
                new XeqY(story[iskyscraper], location[iPortCharles])),
            new And(
                new XeqY(story[iblimp], location[iPortCharles]),
                new XeqY(story[iskyscraper], reporter[iLous]))));

    // 4. South Amboy was not the site of either the beached whale
    // or the skyscraper dedication.
    store.impose(
        new Or(
            new XneqY(location[iSouthAmboy], story[ibeached]),
