 ...
 *     <p>There are four students: Algy, Bertie, Charlie and Digby, who share a flat. Four
 *     newspapers are delivered to the house: the Financial Times, the Guardian, the Daily Express
 *     and the Sun. Each of the students reads all of the newspapers, in particular order and for a
 *     specified amount of time (see below).
 *     <p>Question: Given that Algy gets up at 8:30, Bertie and Charlie at 8:45 and Digby at 9:30,
 *     what is the earliest that they can all set off for college?
 *     <p>Algy Bertie Charlie Digby Guardian 30 75 15 1 FinancialTime (FT) 60 25 10 1 Express 2 3 5
 *     1 Sun 5 10 30 90
...
public class CPvizNewspaper {
  ...
  public void model() {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      algy[i] = new IntVar(store, "algy[" + i + "]", 0, 250);

      // bertie[i] = new Variable(store, "bertie[" + i + "]", 15, 250);
      bertie[i] = new IntVar(store, "bertie[" + i + "]", 0, 250);
      ...
      store.impose(new In(bertie[i], new IntervalDomain(15, 250)));
    ...
    IntVar makespan = new IntVar(store, "makespan", 0, 250);
