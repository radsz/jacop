...
public class SatFlowers extends ExampleFD {
	...
	public void model() {
		...
		for (int i = 0; i < 4; i++) {
			...
			// enable translation
			wrapper.register(husbandD[i]);
			
			wrapper.register(wifeD[i]);
			wrapper.register(occasionD[i]);
			wrapper.register(flowerD[i]);
			wrapper.register(husbandT[i]);
			wrapper.register(wifeT[i]);
			wrapper.register(occasionT[i]);
			wrapper.register(flowerT[i]);
		...
		store.imposeToSat(new Alldifferent(wifeD));
		store.imposeToSat(new Alldifferent(husbandT));
		store.imposeToSat(new Alldifferent(husbandD));
		store.imposeToSat(new Alldifferent(flowerT));
		store.imposeToSat(new Alldifferent(flowerD));
		store.imposeToSat(new Alldifferent(occasionT));
		store.imposeToSat(new Alldifferent(occasionD));
...
//		store.impose(new Alldifferent(occasionD));
	...
	public static void main(String args[]) {
		...
		SatFlowers example = new SatFlowers();
		
		example.model();
		example.wrapper.verbosity = 0;

		// add debug and stat modules
		//StatModule mod = new StatModule(false);
		//example.wrapper.addSolverComponent(mod);
		//example.wrapper.addSolverComponent(new DebugModule());
