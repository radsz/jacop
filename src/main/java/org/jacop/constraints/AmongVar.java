    ...
    @Override public void consistency(Store store) {
        ...
        while (variableQueueY.size() > 0 || firstTimeWhileLoop) {
            ...
            if (this.yGrounded.value() == this.listOfY.length) {
                if (debugAll)
                    System.out
                        .println("All Y were grounded, thus we can pass to simple ve rsion of Among contrians where GAC can be reached");
            ...
            if (debugAll) {
                ...
                System.out
                    .println(" domain of N " + n.domain + " is in [ " + Math.max(n.min(), lb0) + ", " + Math.min(n.max(), ub0) + " ]");
                System.out.println("--------");
            }
            ...
            for (int h = 0; h < (pureUbs).size; h++) {
                ...
                if (inv != null)
                    ...
                    for (int v = inv.min; v <= inv.max; v++) {
                        ...
                        if (debugAll)
                            System.out.println("--- lb[" + v + "] = " + lbTmp);
                        if (debugAll)
                            System.out.println("--- ub[" + v + "] = " + ubTmp);
                        lbV.put(v, lbTmp);
                        ubV.put(v, ubTmp);
                        if (ubTmp > UB)
                            UB = ubTmp;
            ...
            for (int h = 0; h < pureUbs.size; h++) {
                ...
                if (inv != null) {
                    ...
                    for (int v = inv.min; v <= inv.max; v++) {
                        ...
                        if (ubV.get(v) < n.min()) {
                            if (debugAll)
                                System.out.println(v + " must be be present in S");
                        ...
                        if (lbV.get(v) > n.max()) {
                            ...
                            if (debugAll)
                                System.out.println(v + " must be pruned out of all y");
