...
public class DomainHoles extends InternalConstraint { private static Logger logger = LoggerFactory.getLogger(DomainHoles.class);
		...
		for(int i = 0; i < o.dimension+1; i++){
			...
			if(d == o.dimension) {
				...
				if(! dom.contains(c[d])) {
					...
					if(debug)
						logger.info(Arrays.toString(c) + " is in a hole of " + o.coords[d]);
					...
					if(debug){
						logger.info("forbidden domain: " + forbiddenRegion);
					}
				...
				if(debug){
					logger.info(Arrays.toString(c) + " is not in a hole of " + o.coords[d]);
				}
			}
		}
