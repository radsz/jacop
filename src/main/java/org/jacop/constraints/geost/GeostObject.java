        ...
        public boolean consistencyStartPlusDurationEqEnd(Store store) {
            ...
            do {
                ...
                if (start.singleton()) {
                    ...
                    start.domain.in(store.level, start, end.min() - duration.max(), end.max() - duration.min());

                    duration.domain.in(store.level, duration, end.min() - start.max(), end.max() - start.min());

                    end.domain.in(store.level, end, start.min() + duration.min(), start.max() + duration.max());
