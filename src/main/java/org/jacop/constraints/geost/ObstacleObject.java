...
public class ObstacleObject extends ObstacleObjectFrame {
    ...
    public ObstacleObject(Geost geost, GeostObject obstacle, int[] selectedDimensions) {
        ...
        assert obstacle.shapeID
            .singleton() : "Polymorphism not supperted by this simple internal constraint. Use ObstacleObjectFrame instead.";
    ...
    @Override public DBox isFeasible(Geost.SweepDirection min, LexicographicalOrder order, GeostObject o, int currentShape, int[] c) {
        ...
        for (int i = 0; i < obstacle.dimension; i++) {
            if (selectedDimIndex < selectedDimensions.length && selectedDimensions[selectedDimIndex] == i) {
                ...
                final int max = obstacleBB.origin[i] + obstacleBB.length[i] + obstacle.coords[i].min() - otherBB.origin[i];
