...
public class DBox {
    ...
    public boolean containsPoint(int[] pointCoordinates) {
        ...
        if (pointDim == 2) {
            ...
            return !(pointCoordinates[0] < origin[0] || pointCoordinates[0] >= origin[0] + length[0] || pointCoordinates[1] < origin[1]
    ...
    public DBox intersectWith(DBox other) {
        ...
        for (int i = origin.length - 1; i >= 0; i--) { //loop backwards
            ...
            intersectionLength[i] = Math.min(origin[i] + length[i], other.origin[i] + other.length[i]) - intersectionOrigin[i];
    ...
    public DBox intersectWith(DBox other, int[] otherOffset) {
        ...
        for (int i = origin.length - 1; i >= 0; i--) {
            ...
            intersectionLength[i] =
                Math.min(origin[i] + length[i], other.origin[i] + other.length[i] + otherOffset[i]) - intersectionOrigin[i];
    ...
    public DBox intersectWith(int[] offset, DBox other, int[] otherOffset) {
        ...
        for (int i = origin.length - 1; i >= 0; i--) {
            ...
            intersectionLength[i] =
                Math.min(origin[i] + length[i] + offset[i], other.origin[i] + other.length[i] + otherOffset[i]) - intersectionOrigin[i];
            //empty intersection if the size is negative
            if (intersectionLength[i] <= 0)
                return null;
     ...
     */
    public static Collection<DBox> subtractAll(Collection<DBox> source, Collection<DBox> holes, Collection<DBox> result) {
        ...
        assert (resultStep.isEmpty() && !resultWork.isEmpty()) || resultStep.isEmpty() && resultWork
            .isEmpty() : //without this the assertion would fail when subtracting leaves nothing
