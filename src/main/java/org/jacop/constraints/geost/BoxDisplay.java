	...
	*/
    ...
    public void display2DBox(DBox b, Color color, boolean fill) {
        ...
        if (shouldDraw) {
            ...
            g.setColor(color);
            if (fill) {
                g.fillRect(10 + (xCellsShift + b.origin[0]) * pixelsPerUnit,
                    height - (yCellsShift + b.origin[1] + b.length[1]) * pixelsPerUnit,
                    (b.length[0] > 0) ? b.length[0] * pixelsPerUnit - 1 : 1, (b.length[1] > 0) ? b.length[1] * pixelsPerUnit - 1 : 1);
            } else {
                g.drawRect(10 + (xCellsShift + b.origin[0]) * pixelsPerUnit,
                    height - (yCellsShift + b.origin[1] + b.length[1]) * pixelsPerUnit,
                    (b.length[0] > 0) ? b.length[0] * pixelsPerUnit - 1 : 1, (b.length[1] > 0) ? b.length[1] * pixelsPerUnit - 1 : 1);
            }
    ...
    public void display2DGeostObject(Geost geost, GeostObject o, Color c) {
        ...
        for (DBox piece : shape.boxes) {
            for (int i = 0; i < o.dimension; i++) {
                ...
                area.length[i] = coordVar.max() /*+ hole.origin[i]*/ + piece.length[i] - coordVar.min()/*-hole.origin[i]*/;
            }
        ...
        if (draw_bounding_box) {
            ...
            for (int i = 0; i < o.dimension; i++) {
                ...
                area.length[i] = coordVar.max() /*+ hole.origin[i]*/ + shape.boundingBox.length[i] - coordVar.min()/*-hole.origin[i]*/;
            }
    ...
    public void display2DObject(GeostObject o, Shape s) {
        ...
        g.drawRect(10 + ox + bb.origin[0] * pixelsPerUnit, height - oy - (bb.origin[1] + bb.length[1]) * pixelsPerUnit,
            bb.length[0] * pixelsPerUnit, bb.length[1] * pixelsPerUnit);
