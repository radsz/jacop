...
public class Knapsack extends Constraint { private static Logger logger = LoggerFactory.getLogger(Knapsack.class);
	...
	public void removeLevelLate(int level) {
		...
		if (debugAll) {
			...
			if (countRemoveLevel >= REMOVE_INFO_FROM) {
				...
				logger.info("Removelevel for " + level + " is called.");
				logger.info( displayQuantitiesInEfficiencyOrder() );
	...
	private final void blockUpdate() {
		...
		if (debugAll)
			logger.info("Capacity after potential update : " + knapsackCapacity);
		...
		if (debugAll)
			logger.info("Profit after potential update : " + knapsackProfit);
	...
	public void consistency(Store store) {
		...
		if (debugAll)
			logger.info("Entering consistency " + this);
		...
		if (debugAll)
			logger.info("After single item restrictions " + this);
	...
	public void computeForbidden() {
		...
		while (true) {
			...
			if (debugAll)
				logger.info("Forbidden check for " + leaf + " finished. Intrusion weight = " + intrusionWeight);
	...
	public void computeMandatory() {
		...
		while ( true ) {
			...
			if (debugAll)
				logger.info("Mandatory check for " + leaf + " finished. MaxWeight = " + replacableWeight);
	...
	public void impose(Store store) {
		...
		if (debugAll) {
			...
			if (!impositionFailure)
				logger.info("The impose function is completed. ");
			else
				logger.info("The impose function has already detected inconsistency.");
	...
	public void queueVariable(int level, Var v) {
		...
		if (debugAll ) {
			if (countQueueVariable >= QUEUE_INFO_FROM) {
				...
				logger.info("queueVariable is executed for the " + countQueueVariable + "-th time");
				logger.info( displayQuantitiesInEfficiencyOrder() );
