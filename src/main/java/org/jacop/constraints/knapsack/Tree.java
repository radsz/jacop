...
public class Tree {
    ...
    public void initializeComputeMandatory() {
        ...
        currentWeight = 0;
        currentProfit = 0;
        profitFromCriticalLeft = (criticalLeaf.profitOfOne * availableWeightOfCriticalItem) / (double) criticalLeaf.weightOfOne;
    ...
    public int computeReplacableWeight(int weightOfItemChecked, int maxNoOfItems, int profitOfItemChecked, double efficiencyOfItemChecked,
        ...
        if (currentNode == criticalLeaf && efficiencyOfItemChecked * availableWeightOfCriticalItem - profitFromCriticalLeft > profitSlack) {
            ...
            return (int) Math.ceil(
                profitSlack * criticalLeaf.weightOfOne * weightOfItemChecked / (double) (criticalLeaf.weightOfOne * profitOfItemChecked
        ...
        while (!exhaustedRightItems) {
            ...
            if (!rightChild) {
                ...
                double profitEaten = efficiencyOfItemChecked * (currentWeight + currentNode.rightNeighbor.getWSum()) - (currentProfit
                    + currentNode.rightNeighbor.getPSum()) - profitFromCriticalLeft;
            ...
            if (rightChild) {
                ...
                // move right does not exceed slack.
                // @TODO < changed to <=, double check that it causes no problems...
                if (efficiencyOfItemChecked * (currentWeight + currentNode.rightNeighbor.getWSum()) - (currentProfit
                    + currentNode.rightNeighbor.getPSum()) - profitFromCriticalLeft <= profitSlack) {
        ...
        double lastWeight =
            (profitSlack + currentProfit + profitFromCriticalLeft) - currentWeight * profitOfItemChecked / (double) weightOfItemChecked;
    ...
    public void initializeComputeForbidden() {
        ...
        profitFromCriticalTaken =
            (criticalLeaf.getWSum() - availableWeightOfCriticalItem) * criticalLeaf.profitOfOne / (double) criticalLeaf.weightOfOne;
        currentNode = criticalLeaf;
    ...
    public int computeIntrusionWeight(int weightOfItemChecked, int maxNoOfItems, int profitOfItemChecked, double efficiencyOfItemChecked,
        ...
        // If the function can not go beyond critical leaf
        if (currentNode == criticalLeaf && profitFromCriticalTaken - efficiencyOfItemChecked * takenWeightOfCriticalItem > profitSlack) {

            // Playing safe, we return higher value (ceil) to avoid making items forbidden due to rounding errors).
            return (int) Math.ceil(
                profitSlack * criticalLeaf.weightOfOne * weightOfItemChecked / (double) (criticalLeaf.profitOfOne * weightOfItemChecked
        ...
        while (!exhaustedLeftItems) {
            ...
            if (rightChild) {
                ...
                if (profitSlack + efficiencyOfItemChecked * (currentWeight + currentNode.leftNeighbor.getWSum()) - (currentProfit
                    + currentNode.leftNeighbor.getPSum()) - profitFromCriticalTaken < 0) {
        ...
        double lastWeight =
            profitSlack + currentWeight * profitOfItemChecked / (double) weightOfItemChecked - currentProfit - profitFromCriticalTaken;

        if (!exhaustedLeftItems) {

            double efficiencyLoss =
                -profitOfItemChecked / (double) weightOfItemChecked + currentNode.leftNeighbor.getPSum() / (double) currentNode.leftNeighbor
                    .getWSum();
            lastWeight /= efficiencyLoss;
