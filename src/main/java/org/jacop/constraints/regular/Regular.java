...
public class Regular extends Constraint implements UsesQueueVariable, Stateful {
            ...
            for (int i = 0; i < stateNumber; i++) {
                if (outdeg[level][i] > 0) {
                    ...
                    if (s == null) { //If not -> create the state
                        ...
                        if (debugAll)
                            System.out.println(
                                "Create new state q_" + level + i + " with in degree : " + s.inDegree + " and out degree : " + s.outDegree);
                    ...
                    for (int j = 0; j < stateNumber; j++)
                        ...
                        if (outarc[level][i][j] != null && outarc[level][i][j].getSize() > 0) {
                            ...
                            if (suc == null) {
                                ...
                                if (debugAll)
                                    System.out.println(
                                        "Create new state q_" + (level + 1) + j + " with in degree : " + suc.inDegree + " and out degree : "
                                            + suc.outDegree);
                            }
                            ...
                            if (debugAll)
                                System.out.println(
                                    "--  state q_" + level + i + " with in degree : " + s.inDegree + " and out degree : " + s.outDegree);

                            if (debugAll)
                                System.out.println(
                                    "--  state q_" + (level + 1) + j + " with in degree : " + suc.inDegree + " and out degree : "
                                        + suc.outDegree);
                        }
    ...
    public void pruneArc(int varIndex) {
        ...
        for (state = preThisLevelStateNb - 1; state >= 0; state--) {
            ...
            if (debugAll)
                System.out.println(state + ": watch state q_" + varIndex + s.id);
            ...
            for (int i = s.outDegree - 1; i >= 0; i--)
                ...
                if (!s.intersects(domVar, i)) {
                    ...
                    if (debugAll)
                        System.out.println(
                            "must remove transition q_" + varIndex + s.id + " -" + s.sucDomToString(i) + "-> q_" + (varIndex + 1)
                                + s.successors[i].id);
                    ...
                    if (debugAll) {
                        System.out.println(
                            "--  state q_" + s.level + s.id + " with in degree : " + s.inDegree + " and out degree : " + s.outDegree);
                        System.out.println(
                            "--  state q_" + (suc.level) + suc.id + " with in degree : " + suc.inDegree + " and out degree : "
                                + suc.outDegree);
                    }
                    ...
                    if (s.outDegree == 0) {
                        if (debugAll)
                            System.out.println("Move OUT state out of scope : q_" + varIndex + s.id);
        ...
        while (level < this.list.length && cont) {
            ...
            for (state = currentLimit; state < end; state++) {
                ...
                for (int i = s.outDegree - 1; i >= 0; i--) {
                    ...
                    if (debugAll)
                        System.out
                            .println("watch transition q_" + s.level + s.id + " -" + s.sucDomToString(i) + "-> q_" + (suc.level) + suc.id);
    ...
    @Override public void removeLevel(int level) {
        ...
        assert (level > firstConsistencyLevel) : "Constraint has the level at which it has computed its initial state being removed.";
    ...
    @Override public void removeLevelLate(int level) {
        ...
        while (currentTouchedIndex > checkToIndex) {
            ...
            for (int i = prevVal; i < curState.outDegree; i++)
                if (!(successors[i].isActive(activeLevels) && curState.intersects(list[curState.level].domain, i)))
        ...
        if (firstConsistencyCheck) {
            ...
            if (oneSupport) {
                ...
                for (int level = this.list.length - 1; level >= 0; level--) {
                    ...
                    if (debugAll)
                        System.out.println(">>> Variable x_" + level + " had domain " + this.list[level].domain + " and now its " + varDom);
        ...
        if (oneSupport) {
            ...
            for (int level = list.length - 1; level >= 0; level--)
                if (levelHadChanged[level]) {
                    ...
                    if (debugAll)
                        System.out.println(">>> Variable x_" + level + " had domain " + list[level].domain + " and now its " + varDom);
        ...
        for (int i = 0; i < stateLevels.length; i++)
            for (int r = 0; r < this.activeLevels[i].value(); r++) {
                ...
                for (int j = 0; j < s.outDegree; j++) {
                    if (dNames != null)
                        res += "     (q_" + s.level + s.id + ")   edge node    {$" + dNames.get(s.sucDomToString(j)) + "$}    (q_"
                            + s.successors[j].level + s.successors[j].id + ")" + "\n";
                    else
                        res += "     (q_" + s.level + s.id + ")   edge node    {" + s.sucDomToString(j) + "}    (q_" + s.successors[j].level
                            + s.successors[j].id + ")" + "\n";
                }
            }
        res += ";\n";
        ...
        while (currentLevel != -1) {
            ...
            if (nextNodePosition == MDD.TERMINAL) {
                currentState[currentLevel]
                    .addTransition(currentState[list.length], mdd.views[currentLevel].indexToValue[currentOffset[currentLevel]]);
                currentOffset[currentLevel]++;
                continue;
            }
