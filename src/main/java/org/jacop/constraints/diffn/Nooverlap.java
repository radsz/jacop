...
public class Nooverlap extends Constraint {
  ...
  public Nooverlap(ArrayList<? extends ArrayList<? extends IntVar>> rectangle) {
	...
	for (int j = 0; j < r.size(); j++) {
	  this.rectangle[i] = new Rectangle(rectangle.get(i).get(0), rectangle.get(i).get(1), rectangle.get(i).get(2), rectangle.get(i).get(3));
	  this.rectangle[i].index = i;
	  i++;	  
	}
      }
   ...
   */
  public Nooverlap(ArrayList<? extends IntVar> o1,
