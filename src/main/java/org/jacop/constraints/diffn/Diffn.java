...
public class Diffn extends Nooverlap {
   ...
   */
  public Diffn(ArrayList<? extends IntVar> o1,
  ...
  void sweepPruning(Rectangle r, BitSet o, int dim) {
	...
	if (ne == null || ne.type() > profileAdd || e.date < ne.date()) { // check the tasks for pruning only at the end of all profile events
	  ...
	  if (considerR) {
	    ...
	    if (lastBarier == Integer.MAX_VALUE && e.date() >= r.lst(dim) &&
	...
	if (rr.exists()) //(rr.length(oDim).min() > 0 && rr.length(dim).min() > 0)
	  if (startExcluded != Integer.MAX_VALUE) {
	    ...
	    if (startExcluded - 1 <= rr.lst(dim)) {
	      if (debugNarr) 
		System.out.print(">>> Diffn Profile 2. Narrowed " + rr.origin(dim) + " \\ "
