...
public class SinPeqR extends Constraint {
	...
	do {
		...
		double dist = p.min() + FloatDomain.PI/2;
		double noIntervals = - Math.floor(dist / FloatDomain.PI);
		// System.out.println (pDom + " extend left by"); 
		
		for (int i = 0; i < noIntervals; i++) 
		    if (i % 2 == 0) {
			// System.out.println ("1. adding " +  (double)(-(i+1)*FloatDomain.PI - pMax) +".."+ (double)(-(i+1)*FloatDomain.PI - pMin));
			pDom.unionAdapt(- (i+1)*FloatDomain.PI - pMax, - (i+1)*FloatDomain.PI - pMin); 
		    }
		    else {
			// System.out.println ("2. adding " +  (double)(-(i+1)*FloatDomain.PI + pMin) +".."+ (double)(-(i+1)*FloatDomain.PI + pMax));
		...
		double dist = p.max() - FloatDomain.PI/2;
		double noIntervals = Math.ceil(dist / FloatDomain.PI);

		// System.out.println (pDom + " extend rigth by " + noIntervals + " intervals");
		
		for (int i = 0; i < noIntervals; i++) 
		    if (i % 2 == 0) {
			// System.out.println ("3. adding " +  (double)((i+1)*FloatDomain.PI - pMax) +".."+ (double)((i+1)*FloatDomain.PI - pMin));
			pDom.unionAdapt((i+1)*FloatDomain.PI - pMax, (i+1)*FloatDomain.PI - pMin); 
		    }
		    else {
	    ...
	    // if ( (Math.abs(p.min() - pMin) < FloatDomain.precision()) &&
	    // 	 (Math.abs(q.min() - qMin) < FloatDomain.precision()) ) {
	    // 	System.out.println ("Slow convergence p - break" + pMin + ".." + pMax);
	    // 	break;
	    // }
