...
public class FloatVar extends Var {
    ...
    public FloatVar(Store store) {
        this(store, store.getVariableIdPrefix() + idNumber.incrementAndGet(), new FloatIntervalDomain(5));
    }
    ...
    public void domainHasChanged(int event) {
        ...
        assert ((event == IntDomain.ANY && !singleton()) || (event == IntDomain.BOUND && !singleton()) || (event == IntDomain.GROUND
            && singleton())) : "Wrong event generated";
