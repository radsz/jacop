...
public class Options { private static Logger logger = LoggerFactory.getLogger(Options.class);
	...
	public Options(String[] args) {
		...
		if (args.length == 0) {
			logger.info("fz2jacop: no model file specified");
			logger.info("fz2jacop: use --help for more information.");
			System.exit(0);
		}
		else if (args.length == 1) {
			String arg = args[0];
			if (arg.equals("-h") || arg.equals("--help")) {
				logger.info("Usage: java org.jacop.fz.Fz2jacop [<options>] <file>.fzn\n" +
                                    "Options:\n" +
                                    "    -h, --help\n" +
                                    "        Print this message.\n" +
                                    "    -a, --all, --all-solutions\n" +
			...
			while (i<args.length-1) {
				...
				if (args[i].equals("-a") || args[i].equals("--all-solutions") || args[i].equals("--all")) {
					...
					logger.info("fz2jacop: not recognized option " + args[i]);
					i++;
				}
			}
