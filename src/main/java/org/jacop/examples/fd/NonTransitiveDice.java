...
public class NonTransitiveDice extends ExampleFD {
    ...
    @Override public void model() {
        ...
        for (int i = 0; i < noDices; i++)
            for (int j = 0; j < noSides; j++)
                for (int m = 0; m < noSides; m++)
                    wins[i][j][m] = new BooleanVar(store, "win_D" + (i + 1) + "->" + ((i + 2) % noDices) + "F" + j + m, 0, 1);
        ...
        for (int i = 0; i < noDices; i++)
            for (int j = 0; j < noSides; j++)
                for (int m = 0; m < noSides; m++)
                    store.impose(new Reified(new XgtY(faces[noSides * i + j], faces[noSides * ((i + 1) % noDices) + m]), wins[i][j][m]));
        ...
        for (int i = 0; i < noDices; i++) {
            winningSum[i] =
                new IntVar(store, "noWins-d" + (i + 1) + "->d" + ((i + 2) % noDices), noSides * noSides / 2 + 1, noSides * noSides);
