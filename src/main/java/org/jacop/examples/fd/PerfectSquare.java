...
public class PerfectSquare extends ExampleFD {
            ...
            for (int j = 0; j < varsY.length; j++)
                if (i != j) {
                    PrimitiveConstraint[] orArray =
                        {new XlteqY(endX[i], varsX[j]), new XgteqY(varsX[i], endX[j]), new XlteqY(endY[i], varsY[j]),
    ...
    @Override public boolean search() {
        ...
        SelectChoicePoint<IntVar> selectSlave1 =
            new SimpleSelect<IntVar>(varsY, new SmallestMin<IntVar>(), new SmallestDomain<IntVar>(), new IndomainMin<IntVar>());
        ...
        SelectChoicePoint<IntVar> selectMaster =
            new SimpleSelect<IntVar>(varsX, new SmallestMin<IntVar>(), new SmallestDomain<IntVar>(), new IndomainMin<IntVar>());
