    ...
    @Override public void model() {
        ...
        for (int i = 1; i < numbers.length; i++) {
            ...
            for (int j = 0; j < i; j++) {
                ...
                IntVar sub = new IntVar(store, "c" + i + "_" + j, (i - j) * (i - j + 1) / 2, noMarks * noMarks);
        ...
        for (int i = 1; i < noMarks; i++)
            for (int j = 0; j < i; j++)
                ...
                store.impose(new XplusClteqZ(subs.get(index++), (noMarks - 1 - i + j) * (noMarks - i + j) / 2, numbers[noMarks - 1]));
