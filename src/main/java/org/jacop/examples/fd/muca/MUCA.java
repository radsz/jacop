...
public class MUCA extends ExampleFD {
    ...
    @Override public void model() {
        ...
        for (int g = 0; g < noGoods; g++) {
            ...
            IntVar previousPartialSum = new IntVar(store, "initialQuantity_" + g, initialQuantity.get(g), initialQuantity.get(g));
            ...
            for (int i = 0; i < maxNoTransformations; i++) {
                ...
                store.impose(new SumInt(new IntVar[] {previousPartialSum, deltasI[i][g], deltasO[i][g]}, "==", partialSum));
    ...
    public boolean searchSpecial() {
        ...
        SelectChoicePoint<IntVar> select1 =
            new SimpleSelect<IntVar>(bidCosts.toArray(new IntVar[1]), new MaxRegret<IntVar>(), new IndomainMin<IntVar>());

        Search<IntVar> search2 = new DepthFirstSearch<IntVar>();
        SelectChoicePoint<IntVar> select2 = new SimpleSelect<IntVar>(transitions, null, new IndomainMin<IntVar>());
