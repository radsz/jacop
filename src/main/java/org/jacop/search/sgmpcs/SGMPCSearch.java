...
public class SGMPCSearch {
    ...
    public boolean search() {
        ...
        if (trace)
            System.out
                .println("%% Best Cost elite solution is " + bestCostSolution + " with cost " + elite[bestCostSolution][costPosition]);
    ...
    boolean terminationCriteria() {
        ...
        termination =
            (currentTime - searchStartTime > timeOut) || (numberConsecutiveFails > 0 && search.getNumberFails() < search.getFailLimit());

        if (printInfo && termination) {
            System.out.println("%% Termination search fails " + search.getNumberFails() + "(" + search.getFailLimit() + ")");
