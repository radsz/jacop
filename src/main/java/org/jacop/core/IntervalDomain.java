...
public class IntervalDomain extends IntDomain implements Cloneable {
    ...
    @Override public void unionAdapt(int min, int max) {
        ...
        if (size == 0) {
            ...
            for (; i < size; i++) {
                ...
                if ((max + 1 >= intervals[i].min && max <= intervals[i].max + 1) || (min + 1 >= intervals[i].min
                    && min <= intervals[i].max + 1) || (min <= intervals[i].min && intervals[i].max <= max))
                    break;
                ...
                while (enumer.hasMoreElements()) {
                    ...
                    if (i == size) {
                        assert (!isIntersecting(
                            ((SmallDenseDomain) domain).toIntervalDomain())) : "isIntersecting not properly implemented";
                        return false;
                    }
    ...
    @Override public IntervalDomain clone() {
     ...
     */
    @Override public boolean eq(IntDomain domain) {
        ...
        if (domain.domainID() == BoundDomainID) {
            ...
            if (intervals[0].min == domain.min() && intervals[0].max == domain.max())
                return true;
     ...
     */
    @Override public IntDomain intersect(IntDomain domain) {
    ...
    @Override public IntervalDomain subtract(int min, int max) {
        ...
        if (domain.domainID() == IntervalDomainID) {
            ...
            while (true) {
                ...
                if (i1 == max1) {
                    ...
                    if (currentDomain1.max <= currentDomain2.max && currentDomain1.max + 1 >= currentDomain2.min) {
                ...
                if (i2 == max2) {
                    ...
                    if (currentDomain2.max <= currentDomain1.max && currentDomain2.max + 1 >= currentDomain1.min) {
                        result.unionAdapt(new Interval(min, currentDomain1.max));
                        i1++;
                    } else {
                        result.unionAdapt(new Interval(min, currentDomain2.max));
    ...
    @Override public void in(int storeLevel, Var var, IntDomain domain) {
        ...
        if (domain.isSparseRepresentation()) {
            ...
            if (isSparseRepresentation()) {
                ...
                // traverse within while loop until certain that change will occur
                while (intervals[pointer1].min >= domain.getInterval(pointer2).min && intervals[pointer1].max <= domain
                    .getInterval(pointer2).max && ++pointer1 < size) {
    ...
    @Override public void inShift(int storeLevel, Var var, IntDomain domain, int shift) {
                ...
                if (interval1Max < interval2Min) {
                    ...
                    if (interval1Min <= interval2Min) {
                        ...
                        if (interval1Max <= interval2Max) {
                            ...
                            result.unionAdapt(new Interval(inputIntervals[pointer2].min + shift, inputIntervals[pointer2].max + shift));
        ...
        if (domain.isSparseRepresentation()) {
            ...
            if (isSparseRepresentation()) {
                ...
                // Chance for no event
                while (pointer2 < inputSize && domain.getInterval(pointer2).max + shift < intervals[pointer1].min)
                    pointer2++;
                ...
                while (intervals[pointer1].min >= domain.getInterval(pointer2).min + shift
                    && intervals[pointer1].max <= domain.getInterval(pointer2).max + shift && ++pointer1 < size) {

                    while (intervals[pointer1].max > domain.getInterval(pointer2).max + shift && ++pointer2 < inputSize)
                        ;
                    ...
                    if (interval1Max < interval2Min) {
                        ...
                        if (interval1Min <= interval2Min) {
                            ...
                            if (interval1Max <= interval2Max) {
                                ...
                                result.unionAdapt(
                                    new Interval(domain.getInterval(pointer2).min + shift, domain.getInterval(pointer2).max + shift));
                                pointer2++;
    ...
    @Override public void subtractAdapt(int value) {
    ...
    @Override public void subtractAdapt(int minValue, int maxValue) {
    ...
    @Override public int intersectAdapt(IntDomain domain) {
        ...
        if (domain.domainID() == SmallDenseDomainID) {
            ...
            if (input.min == this.min() && input.max() == this.max() && input.getSize() == this.getSize()
    ...
    @Override public int unionAdapt(IntDomain union) {
