...
public class MutableDomain implements MutableVar { private static Logger logger = LoggerFactory.getLogger(MutableDomain.class);
	...
	public void update(MutableVarValue val) {
		...
		if (value.stamp == store.level) {
			...
			if (debug)
				logger.info("1. Level: " + store.level + ", IN " + value + ", New " + val);
			...
			if (debug)
				logger.info("2. Level: " + store.level + ", IN " + this + ", New " + val);
