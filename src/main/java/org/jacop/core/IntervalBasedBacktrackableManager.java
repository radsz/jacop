...
public class IntervalBasedBacktrackableManager extends SimpleBacktrackableManager { private static Logger logger = LoggerFactory.getLogger(IntervalBasedBacktrackableManager.class);
	...
	public void addChanged(int index) {
		...
		if (trailContainsAllChanges) {
			...
			if (debug) {
				...
				logger.info("Level info " + levelInfo);
				logger.info("Intervals? " + intervalBasedTrail);
				logger.info("LastTrail " + trail.get(trail.size() - 1));
	...
	public void setLevel(int level) {
		...
		if (debug) {
			...
			logger.info("Level being set" + level);
			logger.info("Last Level info " + levelInfo);
			logger.info("Intervals? " + intervalBasedTrail);
			if (trail.size() != 0)
				logger.info("LastTrail " + trail.get(trail.size() - 1));
			logger.info(super.toString());
	...
	public void removeLevel(int removedLevel) {
		...
		if (debug)
			logger.info("Remove level count " + removeCount);
