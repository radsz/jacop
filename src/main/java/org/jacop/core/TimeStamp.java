...
public class TimeStamp<T> {
    ...
    @SuppressWarnings("unchecked") public transient T[] values = (T[]) new Object[10];
    ...
    public void update(T val) {
        ...
        assert (stamps[pointer4Last] <= store.level) : "Error - Timestamp" + this + "has greater level than store " + "- missing remove";

        if (stamps[pointer4Last] == store.level) {
            if (debug)
                System.out.print("1. Level: " + store.level + ", In " + this + ",  New value " + val + "replaces old");
