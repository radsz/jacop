...
public class RegularExpressionParser {
        ...
        while (contin) {
            ...
            switch (token) {
                case LexicalAnalyzer.PLUS:
                    ...
                    if (c.getType() == RegularExpressionParser.Sum && c2.getType() == RegularExpressionParser.Sum)
                        ((Sum) c).addSum((Sum) c2);
                ...
                case LexicalAnalyzer.WORD:
                    ...
                    if (token != LexicalAnalyzer.RIGHT_PAREN && token != LexicalAnalyzer.EOF) {
                        if (token != LexicalAnalyzer.DOT && token != LexicalAnalyzer.STAR && token != LexicalAnalyzer.PLUS) {
                            // print error message and throw SyntaxException
    ...
    class Concatination extends Expression {
        ...
        @Override public FSM parseToFSM() {
