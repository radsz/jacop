 ...
 * Lengauer and Tarjan algorithm presented in paper "A Fast Algorithm for Finding Dominators in a Flowgraph", 
...
public class LengauerTarjan {
	...
	for (int i = n-1; i > 0; i--) {
		...
		// step_3:
		if (debug)
		    System.out.println("bucket[" + parent[w]+"] = "+ bucket[parent[w]]);
	...
	if (debug)
		...
		System.out.println("ancestor["+i+"] = "+ ancestor[i]);
		System.out.println("semi["+i+"] = "+ semi[i]);
		System.out.println("dom["+i+"] = "+ dom[i]);
	    }
    ...
    void printGraph(PrintStream p) {
	p.println ("{");
	
	for (int i = 0; i < succ.length; i++) {
	    
	    for (int node = succ[i].nextSetBit(0); node >= 0; node = succ[i].nextSetBit(node + 1)) {
		String  arc = "\"";
		arc += i;
		arc +=  "\" -> ";
		arc +=  "\"";
