...
public class MDD {
   ...
   * This function encodes table constraint in the form of a table
    ...
    private void shrink() {
        ...
        if (reducedNodes.isEmpty()) {
            ...
            while (!reducedNodes.isEmpty()) {
                ...
                System.arraycopy(diagram, range[shiftPosition - 1] + previousShift, shrankDiagram, positionInShrankDiagram,
        ...
        for (int[] tuple : table) {
            ...
            for (int i = 0; i < tuple.length; i++) {
                ...
                assert (positions[i] != -1) :
                    "value specified by tuple " + Arrays.asList(tuple) + "for variable no. " + i + "is already outside its initial domain.";
     ...
     */
    @SuppressWarnings("unchecked") public void reduce() {
