...
public class SearchItem <T extends Var> implements ParserTreeConstants {
    ...
    public ComparatorsVar<IntVar> getVarSelect() {
        ...
        if (var_selection_heuristic == null)
            return new ComparatorsVar<IntVar>(null);
        else switch (var_selection_heuristic) {
                case "input_order":
                    return new ComparatorsVar<IntVar>(null);
                case "random":
                ...
                case "impact":
                    return new ComparatorsVar<IntVar>(new ActivityMax<IntVar>(store), new MostConstrainedStatic<IntVar>());
                case "dom_w_deg":
    ...
    public ComparatorsVar<FloatVar> getFloatVarSelect() {
        ...
        if (var_selection_heuristic == null)
            return new ComparatorsVar<FloatVar>(null);
                ...
                case "impact":
                    return new ComparatorsVar<FloatVar>(new ActivityMax<FloatVar>(store), new MostConstrainedStatic<FloatVar>());
                case "afc_max":
                    // does not follow flatzinc standard (JaCoP specific) ;)
    ...
    ComparatorsVar<SetVar> getSetVarSelect() {
        ...
        if (var_selection_heuristic == null)
            return new ComparatorsVar<SetVar>(null);
                ...
                case "impact":
                    return new ComparatorsVar<SetVar>(new ActivityMax<SetVar>(store), new MostConstrainedStatic<SetVar>());
                case "afc_max":
                    // does not follow flatzinc standard (JaCoP specific) ;)
                ...
                case "most_constrained":
                  return new ComparatorsVar<SetVar>(new MinGlbCard<SetVar>(), new MostConstrainedStatic<SetVar>());
                case "largest":
        ...
        if (((ASTAnnotation)node).getAnnId() == "$vector") {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (((SimpleNode)n.jjtGetChild(0)).getId() == JJTSETLITERAL) {
                    SetVar el = new SetVar(store); // ground set already defined on variable list; define empty set for search
                    aa[i] = el;
                } else {
                    ASTScalarFlatExpr child = (ASTScalarFlatExpr) n.jjtGetChild(0);
