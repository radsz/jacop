   ...
   * it has expensive consistency method that do not need to be executed during model
    ...
    void gen_jacop_networkflow(SimpleNode node) {
        ...
        for (int i = 0; i < flow.length; i++) {
            net.addArc(netNode[arc[2 * i] - minNode], netNode[arc[2 * i + 1] - minNode], weight[i], flow[i]);
        }
    ...
    void gen_jacop_all_equal_int_reif(SimpleNode node) {
        ...
        if (x.length == 2)
            ...
            support.pose(new Reified(new And(new XeqY[] {new XeqY(x[0], x[1]), new XeqY(x[1], x[2])}), b));
        else
